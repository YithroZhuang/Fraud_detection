...
def build_model(BATCH_SIZE,VOCAB_SIZE,EMBED_SIZE,NUM_SAMPLED):
    ...
    with tf.name_scope('data'):
        center_node = tf.placeholder(tf.int32, shape=[None], name='center_node')
        context_node = tf.placeholder(tf.int32, shape=[None, 1], name='context_node')
    ...
    print('********************debug***************',center_node.shape)
    print('********************debug***************',context_node.shape)
    ...
    with tf.name_scope('embedding_matrix'):
        embed_matrix = tf.Variable(tf.random_uniform([VOCAB_SIZE, EMBED_SIZE], 0., 1.), 
                            name='embed_matrix')
    ...
    *epoch index starts from 1! not 0.
    ...
    with tf.Session() as sess:
        ...
        while (dataset.epoch <= NUM_EPOCHS):
            ...
            negative_samples  = dataset.get_negative_samples(pos_index=center_node_batch,num_negatives=NUM_SAMPLED,care_type=care_type)
            ...
            if dataset.epoch - current_epoch > 0:
                ...
                save_path = saver.save(sess, model_path)
                print("Model saved in file: %s" % save_path)
                print('Average loss in this epoch: {:.5f}'.format(total_loss / iteration))
                total_loss = 0.0
                iteration=0
